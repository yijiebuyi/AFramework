package com.callme.platform.widget.nrl;
...
public class NsRefreshLayout extends FrameLayout {
     ...
     * 误差值
     */
    private final float mErrorValue = 1.5F;
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        if (resId == -1) {
            ...
            mLoadViewBgColor = ContextCompat.getColor(getContext(), resId);
        }
        ...
        if (resId == -1) {
            ...
            mLoadViewTextColor = ContextCompat.getColor(getContext(), resId);
        }
        ...
        if (resId == -1) {
            ...
            mProgressBgColor = ContextCompat.getColor(getContext(), resId);
        }

        //进度条颜色
        resId = ta.getResourceId(R.styleable.NsRefreshLayout_progress_bar_color, -1);
        if (resId == -1) {
            mProgressColor = ta.getColor(R.styleable.NsRefreshLayout_progress_bar_color, Color.parseColor("#4baafa"));
        } else {
    ...
    private boolean handleScroll(float distanceY) {
        if (!canChildScrollUp() && mCurrentAction == ACTION_PULL_DOWN_REFRESH &&
            ...
            if (lp.height < loadingViewOverHeight - mErrorValue) {
                headerView.setLoadText(TextUtils.isEmpty(mPullRefreshText) ?
                        getContext().getString(R.string.default_pull_refresh_text) : mPullRefreshText);
            } else {
    ...
    private void resetContentView() {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (child != null && child instanceof ListView && child.isShown()) {
                mContentView = child;
            }
        }
    }
    ...
    public boolean canChildScrollUp() {
        ...
        if (Build.VERSION.SDK_INT < 14) {
            if (mContentView instanceof AbsListView) {
                ...
                return mContentView.canScrollVertically(-1) || mContentView.getScrollY() > 0;
            }
        } else {
