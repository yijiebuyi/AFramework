 ...
 */
public abstract class RAbsSwipeAdapter<B, H> extends BaseAdapter implements
...
//		}
    ...
    public RAbsSwipeAdapter(Context context, PullToRefreshSwipeListView listView) {
        mContext = context;
        mInflater = (LayoutInflater) mContext
                .getSystemService(Context.LAYOUT_INFLATER_SERVICE);
...
//		soundListener.addSoundEvent(State.REFRESHING, R.raw.refresh);
    ...
    protected void doRequest() {
        mCurrentPage++;
        Call<ResultBean<List<B>>> call = initCall();
        Call<ResultBean<B>> oldCall = initOldCall();

        if (call != null && oldCall == null) {
            makeCall(call);
        } else if (oldCall != null && call == null) {
            makeOldCall(oldCall);
        }
    }
     ...
     * @param call
     */
    private final void makeOldCall(Call<ResultBean<B>> call) {
        if (call != null) {
    ...
    private final void makeCall(Call<ResultBean<List<B>>> call) {
        if (call != null) {
            call.enqueue(new Callback<ResultBean<List<B>>>() {
                @Override
                public void onResponse(Call<ResultBean<List<B>>> call, Response<ResultBean<List<B>>> response) {
                    if (response != null && response.code() == 200) {
                        ResultBean<List<B>> result = response.body();
                        if (result != null && result.code == 0) {
                            List<B> beans = result.data;
     ...
     * @return
     */
    protected Call<ResultBean<B>> initOldCall() {
        return null;
    }
     ...
     * @since Version 3.0.0
     */
    protected abstract Call<ResultBean<List<B>>> initCall();
    ...
    protected void addEmptyView() {
        ...
        if (mEmptyIcon == -1 && mEmptyTitle == -1 && mEmptyTip <= 0 && TextUtils.isEmpty(mEmptyTipStr)) {
